...
} from './CalculationPreview';

import {
  ExpenseWarningsPanel,
  type DisplayWarning,
} from './ExpenseWarningsPanel';

const TAX_PRESETS = [
  { label: 'Sin impuestos (0%)', value: 0 },
  { label: 'Superreducido (4%)', value: 400 },
  { label: 'Reducido (10%)', value: 1000 },
  { label: 'General (21%)', value: 2100 },
];
...
function customFieldError(
  ...
  const message =
    errors.customFields?.[fieldId]?.message;
...
export function ExpenseForm({
  ...
  const defaults: ExpenseFormValues =
    useMemo(
      () => ({
        ...
        taxRateBps:
          expense?.tax.rateBps ?? 2100,

        taxIncluded:
          expense?.tax.included ?? false,
  ...
  const schema = useMemo(
    () =>
      makeExpenseSchema(customFields),
    [customFields]
  );
  ...
  useEffect(() => {
    if (
      ...
      setError('amountDecimal', {
        ...
        message: `Este importe no es válido para ${values.currency}.`,
      });
    }
  }, [
    values.currency,
  ...
  const localWarnings:
    DisplayWarning[] = useMemo(() => {
      if (
        !local ||
        !warningRulesQuery.data
      )
        return [];

      return evaluateWarningsLocally(
        warningRulesQuery.data,
        {
  ...
  async function submit(
    ...
    try {
      await onSubmit({
        ...
        merchant:
          formValues.merchant.trim(),

        expenseDate:
          formValues.expenseDate,

        // La API requiere enviar la categoría, usando null cuando no hay ninguna seleccionad
        categoryId:
        ...
        amountDecimal:
          formValues.amountDecimal,

        tax: {
          rateBps:
            formValues.taxRateBps,

          included:
            formValues.taxIncluded,
        },
  ...
  return (
    ...
    >
      <div className="space-y-6 lg:col-span-3">
        <Card>
          <CardHeader title="Detalles del gasto" />
              ...
              <Select
                ...
                hint={
                  categoriesQuery.isSuccess &&
                    categories.length ===
                    0
                    ? 'Aún no existen categorías. Un administrador puede crearlas en Ajustes.'
                    : 'La categoría determina qué reglas de aviso se aplican.'
                }
                {...register(
                  'categoryId',
                  {
        ...
        <Card>
          <CardHeader
            title="Importe e impuestos"
            description="Los importes se mantienen como texto y se convierten mediante aritmética de enteros, sin utilizar números de coma flotante."
          />
          ...
          <CardBody className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {useCustomRate && (
                <Input
                  label="Tipo personalizado (puntos básicos)"
                  type="number"
                  min={0}
                  max={
                    MAX_TAX_RATE_BPS
          ...
          <CardBody className="space-y-4">
            ...
            {previewQuery.isError && (
              <AlertBanner tone="info">
                ...
                se muestra la estimación
                local. Los valores
                oficiales se calcularán
                igualmente al guardar.
              </AlertBanner>
            )}
          </CardBody>
        </Card>
        ...
        <Card>
          ...
          <CardBody>
            <ExpenseWarningsPanel
              ...
              isPreview
              emptyMessage="Este importe no activa ningún aviso."
            />
            ...
            {blocking && (
              <div className="mt-3">
                <AlertBanner
                  tone="error"
                  title="Aviso bloqueante"
                >
                  Puedes guardar este
                  gasto, pero el informe
                  no se podrá enviar
                  mientras exista un
                  aviso bloqueante.
                </AlertBanner>
